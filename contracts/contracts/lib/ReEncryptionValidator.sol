...
library ReEncryptionValidator {
    ...
    uint256 public constant UMBRAL_PARAMETER_U_XCOORD =
        0x03c98795773ff1c241fc0b1cced85e80f8366581dda5c9452175ebd41385fa1f;
    uint256 public constant UMBRAL_PARAMETER_U_YCOORD =
        0x7880ed56962d7c0ae44d6f14bb53b5fe64b31ea44a41d0316f3a598778f0f936;
    ...
    // -2 mod FIELD_ORDER
    uint256 internal constant MINUS_2 =
        0xfffffffffffffffffffffffffffffffffffffffffffffffffffffffefffffc2d;

    // (-1/2) mod FIELD_ORDER
    uint256 internal constant MINUS_ONE_HALF =
    ...
    function validateCFrag(
        ...
        UmbralDeserializer.PreComputedData memory _precomputed = _precomputedBytes
            .toPreComputedData();
        ...
        require(
            isOnCurve(_precomputed.pointEZxCoord, _precomputed.pointEZyCoord),
        ...
        require(
            isOnCurve(_precomputed.pointE1HxCoord, _precomputed.pointE1HyCoord),
        ...
        require(
            isOnCurve(_precomputed.pointVZxCoord, _precomputed.pointVZyCoord),
        ...
        require(
            isOnCurve(_precomputed.pointV1HxCoord, _precomputed.pointV1HyCoord),
        ...
        require(
            isOnCurve(_precomputed.pointUZxCoord, _precomputed.pointUZyCoord),
        ...
        require(
            isOnCurve(_precomputed.pointU1HxCoord, _precomputed.pointU1HyCoord),
    ...
    function extendedKeccakToBN(bytes memory _data) internal pure returns (uint256) {
        ...
        (upper, lower) = (
            keccak256(abi.encodePacked(uint8(0x00), input)),
    ...
    function ecmulVerify(
        ...
        address signer = ecrecover(
            ...
            uint8(27 + (y1 % 2)),
            bytes32(x1),
            bytes32(mulmod(scalar, x1, curve_order))
        );
    ...
    function eqAffineJacobian(
        ...
        return
            mulmod(P[0], Q_z_squared, p) == Q[0] &&
            mulmod(P[1], mulmod(Q_z_squared, Qz, p), p) == Q[1];
    }
    ...
    function doubleJacobian(uint256[3] memory P) internal pure returns (uint256[3] memory Q) {
        ...
        Q[1] = addmod(
            ...
            mulmod(MINUS_ONE_HALF, mulmod(_4yy, _4yy, p), p),
            p
        );
        Q[2] = mulmod(_2y, z, p);
    }
