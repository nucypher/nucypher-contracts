...
library UmbralDeserializer {
    ...
    uint256 internal constant CORRECTNESS_PROOF_SIZE =
        4 * POINT_SIZE + BIGNUM_SIZE + SIGNATURE_SIZE;
     ...
     */
    function toCapsule(bytes memory _capsuleBytes) internal pure returns (Capsule memory capsule) {
    ...
    function copyBytes(
        ...
        assembly {
            ...
            for {
                ...
                // start at arr + 32 -> first byte corresponds to length
                i := add(i, 1)
            } {
                let offset := mul(i, 32)
