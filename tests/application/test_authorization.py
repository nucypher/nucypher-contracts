...
def test_control_allow_list(
    accounts, threshold_staking, taco_application, child_application, chain
):
    """
    Tests for method: controlAllowList
    """
    ...
    with ape.reverts():
        taco_application.controlAllowList([staking_provider_1], True, sender=staking_provider_1)
    assert not taco_application.allowList(staking_provider_1)
    assert not taco_application.allowList(staking_provider_2)
    assert not taco_application.allowList(staking_provider_3)
    assert not taco_application.allowList(staking_provider_4)

    taco_application.controlAllowList([staking_provider_1], True, sender=creator)
    assert taco_application.allowList(staking_provider_1)
    assert not taco_application.allowList(staking_provider_2)
    assert not taco_application.allowList(staking_provider_3)
    assert not taco_application.allowList(staking_provider_4)

    taco_application.controlAllowList(
        [staking_provider_4, staking_provider_1, staking_provider_2], True, sender=creator
    )
    assert taco_application.allowList(staking_provider_1)
    assert taco_application.allowList(staking_provider_2)
    assert not taco_application.allowList(staking_provider_3)
    assert taco_application.allowList(staking_provider_4)

    taco_application.controlAllowList(
        [staking_provider_3, staking_provider_1, staking_provider_2], False, sender=creator
    )
    assert not taco_application.allowList(staking_provider_1)
    assert not taco_application.allowList(staking_provider_2)
    assert not taco_application.allowList(staking_provider_3)
    ...
    taco_application.controlAllowList([staking_provider_4], False, sender=creator)
    assert not taco_application.allowList(staking_provider_1)
    assert not taco_application.allowList(staking_provider_2)
    assert not taco_application.allowList(staking_provider_3)
    assert not taco_application.allowList(staking_provider_4)
...
def test_migrate(accounts, threshold_staking, taco_application, child_application, chain):
    """
    Tests for method: migrateFromThreshold
    """

    (creator, staking_provider, staking_provider_2, owner, beneficiary, authorizer) = accounts[:6]
    minimum_authorization = MIN_AUTHORIZATION
    value = 3 * minimum_authorization

    # Nothing to migrate
    with ape.reverts("Not owner or provider"):
        taco_application.migrateFromThreshold(staking_provider, sender=staking_provider)
    ...
    with ape.reverts("Not owner or provider"):
        taco_application.migrateFromThreshold(staking_provider, sender=creator)
    with ape.reverts("Migration is not allowed"):
        taco_application.migrateFromThreshold(staking_provider, sender=staking_provider)
    taco_application.controlAllowList([staking_provider], True, sender=creator)

    assert taco_application.rolesOf(staking_provider) == (ZERO_ADDRESS, ZERO_ADDRESS)
    tx = taco_application.migrateFromThreshold(staking_provider, sender=owner)
    ...
    with ape.reverts("Migration completed"):
        taco_application.migrateFromThreshold(staking_provider, sender=staking_provider)

    taco_application.controlAllowList([staking_provider_2], True, sender=creator)
    ...
    with ape.reverts("Not an active staker"):
        taco_application.migrateFromThreshold(staking_provider_2, sender=staking_provider_2)
    threshold_staking.authorizationIncreased(staking_provider_2, 0, value, sender=creator)

    threshold_staking.involuntaryAuthorizationDecrease(
        staking_provider_2, value, minimum_authorization // 2, sender=creator
    )

    assert taco_application.rolesOf(staking_provider_2) == (ZERO_ADDRESS, ZERO_ADDRESS)
    tx = taco_application.migrateFromThreshold(staking_provider_2, sender=staking_provider_2)
    assert taco_application.rolesOf(staking_provider_2) == (staking_provider_2, staking_provider_2)
    assert taco_application.authorizedStake(staking_provider_2) == minimum_authorization // 2
    assert (
