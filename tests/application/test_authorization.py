...
def test_authorization_increase(accounts, threshold_staking, pre_application):
    ...
    with ape.reverts():
        pre_application.authorizationIncreased(staking_provider, 0, value, sender=creator)
    ...
    assert pre_application.stakingProviderInfo(staking_provider)[AUTHORIZATION_SLOT] == value
    assert pre_application.authorizedOverall() == 0
    assert pre_application.authorizedStake(staking_provider) == value
    assert pre_application.isAuthorized(staking_provider)

    # Check that all events are emitted
    events = pre_application.AuthorizationIncreased.from_receipt(tx)
    assert len(events) == 1
    event = events[0]
    ...
    assert (
        pre_application.stakingProviderInfo(staking_provider)[AUTHORIZATION_SLOT] == authorization
    )
    assert pre_application.authorizedOverall() == authorization
    assert pre_application.authorizedStake(staking_provider) == authorization
    assert pre_application.isAuthorized(staking_provider)
    ...
    assert pre_application.stakingProviderInfo(staking_provider)[AUTHORIZATION_SLOT] == value
    assert pre_application.authorizedOverall() == value
    assert pre_application.authorizedStake(staking_provider) == value
    assert pre_application.isAuthorized(staking_provider)
...
def test_involuntary_authorization_decrease(accounts, threshold_staking, pre_application):
    """
    ...
    with ape.reverts():
        pre_application.involuntaryAuthorizationDecrease(staking_provider, value, 0, sender=creator)
    ...
    assert pre_application.authorizedStake(staking_provider) == authorization
    assert pre_application.isAuthorized(staking_provider)
    assert not pre_application.isOperatorConfirmed(staking_provider)

    events = pre_application.AuthorizationInvoluntaryDecreased.from_receipt(tx)
    assert len(events) == 1
    event = events[0]
    assert event["stakingProvider"] == staking_provider
    assert event["fromAmount"] == value
    ...
    assert (
        pre_application.stakingProviderInfo(staking_provider)[DEAUTHORIZING_SLOT] == authorization
    )
    assert pre_application.authorizedOverall() == 0
    assert pre_application.authorizedStake(staking_provider) == authorization
    assert pre_application.isAuthorized(staking_provider)
    ...
    assert pre_application.authorizedStake(staking_provider) == authorization
    assert pre_application.isAuthorized(staking_provider)
    assert pre_application.isOperatorConfirmed(staking_provider)
    assert pre_application.getOperatorFromStakingProvider(staking_provider) == staking_provider
    assert pre_application.stakingProviderFromOperator(staking_provider) == staking_provider
    ...
    assert pre_application.stakingProviderInfo(staking_provider)[AUTHORIZATION_SLOT] == 0
    assert pre_application.stakingProviderInfo(staking_provider)[DEAUTHORIZING_SLOT] == 0
    assert pre_application.authorizedOverall() == 0
    assert pre_application.authorizedStake(staking_provider) == 0
    assert not pre_application.isAuthorized(staking_provider)
    assert not pre_application.isOperatorConfirmed(staking_provider)
    assert pre_application.getOperatorFromStakingProvider(staking_provider) == ZERO_ADDRESS
    assert pre_application.stakingProviderFromOperator(staking_provider) == ZERO_ADDRESS
...
def test_authorization_decrease_request(accounts, threshold_staking, pre_application, chain):
    """
    ...
    # Can't call `involuntaryAuthorizationDecrease` directly
    with ape.reverts():
        pre_application.authorizationDecreaseRequested(staking_provider, value, 0, sender=creator)
    ...
    assert pre_application.stakingProviderInfo(staking_provider)[AUTHORIZATION_SLOT] == value
    assert (
        pre_application.stakingProviderInfo(staking_provider)[DEAUTHORIZING_SLOT]
        == minimum_authorization + 1
    )
    end_deauthorization = timestamp + deauthorization_duration
    assert (
        pre_application.stakingProviderInfo(staking_provider)[END_DEAUTHORIZATION_SLOT]
        == end_deauthorization
    )
    assert pre_application.authorizedOverall() == 0
    assert pre_application.authorizedStake(staking_provider) == value
    ...
    events = pre_application.AuthorizationDecreaseRequested.from_receipt(tx)
    assert len(events) == 1
    event = events[0]
    assert event["stakingProvider"] == staking_provider
    assert event["fromAmount"] == value
    ...
    timestamp = chain.pending_timestamp - 1
    assert pre_application.stakingProviderInfo(staking_provider)[AUTHORIZATION_SLOT] == value
    assert pre_application.stakingProviderInfo(staking_provider)[DEAUTHORIZING_SLOT] == value
    end_deauthorization = timestamp + deauthorization_duration
    assert (
        pre_application.stakingProviderInfo(staking_provider)[END_DEAUTHORIZATION_SLOT]
        == end_deauthorization
    ...
    assert pre_application.stakingProviderInfo(staking_provider)[AUTHORIZATION_SLOT] == value // 2
    assert pre_application.stakingProviderInfo(staking_provider)[DEAUTHORIZING_SLOT] == value // 2
    assert pre_application.authorizedOverall() == value // 2
    assert pre_application.authorizedStake(staking_provider) == value // 2
...
def test_finish_authorization_decrease(accounts, threshold_staking, pre_application, chain):
    """
    ...
    with ape.reverts():
        pre_application.finishAuthorizationDecrease(staking_provider, sender=creator)
    ...
    tx = pre_application.finishAuthorizationDecrease(staking_provider, sender=creator)
    assert pre_application.stakingProviderInfo(staking_provider)[AUTHORIZATION_SLOT] == new_value
    assert pre_application.stakingProviderInfo(staking_provider)[DEAUTHORIZING_SLOT] == 0
    assert pre_application.stakingProviderInfo(staking_provider)[END_DEAUTHORIZATION_SLOT] == 0
    assert pre_application.authorizedOverall() == 0
    assert pre_application.authorizedStake(staking_provider) == new_value
    assert pre_application.isAuthorized(staking_provider)
    assert threshold_staking.authorizedStake(staking_provider, pre_application.address) == new_value

    events = pre_application.AuthorizationDecreaseApproved.from_receipt(tx)
    assert len(events) == 1
    event = events[0]
    assert event["stakingProvider"] == staking_provider
    assert event["fromAmount"] == value
    ...
    assert pre_application.getOperatorFromStakingProvider(staking_provider) == staking_provider
    assert pre_application.stakingProviderFromOperator(staking_provider) == staking_provider
    assert pre_application.authorizedOverall() == new_value
    assert pre_application.authorizedStake(staking_provider) == new_value
    assert pre_application.isAuthorized(staking_provider)
    assert pre_application.isOperatorConfirmed(staking_provider)
    assert threshold_staking.authorizedStake(staking_provider, pre_application.address) == new_value
    ...
    assert not pre_application.isAuthorized(staking_provider)
    assert not pre_application.isOperatorConfirmed(staking_provider)
    assert threshold_staking.authorizedStake(staking_provider, pre_application.address) == 0
...
def test_resync(accounts, threshold_staking, pre_application):
    """
    ...
    with ape.reverts():
        pre_application.resynchronizeAuthorization(staking_provider, sender=creator)
    ...
    tx = pre_application.resynchronizeAuthorization(staking_provider, sender=creator)
    assert pre_application.stakingProviderInfo(staking_provider)[AUTHORIZATION_SLOT] == new_value
    assert pre_application.authorizedOverall() == 0
    assert pre_application.authorizedStake(staking_provider) == new_value
    assert pre_application.isAuthorized(staking_provider)

    events = pre_application.AuthorizationReSynchronized.from_receipt(tx)
    assert len(events) == 1
    event = events[0]
    assert event["stakingProvider"] == staking_provider
    assert event["fromAmount"] == value
    ...
    tx = pre_application.resynchronizeAuthorization(staking_provider, sender=creator)
    assert pre_application.stakingProviderInfo(staking_provider)[AUTHORIZATION_SLOT] == new_value
    assert pre_application.stakingProviderInfo(staking_provider)[DEAUTHORIZING_SLOT] == new_value
    assert pre_application.authorizedOverall() == new_value
    assert pre_application.getOperatorFromStakingProvider(staking_provider) == staking_provider
    assert pre_application.stakingProviderFromOperator(staking_provider) == staking_provider
    assert pre_application.authorizedOverall() == new_value
