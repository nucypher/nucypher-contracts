...
def simple_pre_application(SimplePREApplication, accounts):
    return accounts[0].deploy(SimplePREApplication)


@pytest.fixture(scope="session")
def testnet_threshold_staking(TestnetThresholdStaking, accounts):
    return accounts[0].deploy(TestnetThresholdStaking)
...
def test_set_application(pre_application):
